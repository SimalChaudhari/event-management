...
const NavCollapse = (props) => {
    ...
    useEffect(() => {
        ...
        // Check if previous pathname matched this collapse
        const prevPathname = prevPathnameRef.current;
        let prevShouldBeOpen = false;
        
        if (prevPathname) {
            // Special handling for Module collapse
            if (props.collapse.id === 'Module') {
                prevShouldBeOpen = prevPathname === '/categories' ||
                    prevPathname.includes('/add-category') ||
                    prevPathname.includes('/edit-category') ||
                    prevPathname.includes('/view-category') ||
                    prevPathname.startsWith('/surveys') ||
                    prevPathname.startsWith('/polls') ||
                    prevPathname.startsWith('/order') ||
                    prevPathname.startsWith('/engagements');
            } else if (props.collapse.id === 'Media Library') {
                // Special handling for Media Library collapse - match gallery routes
                prevShouldBeOpen = prevPathname === '/media-manager/gallery' ||
                    prevPathname.startsWith('/events/gallery');
            } else if (props.collapse.id === 'events') {
                // For Events, exclude category and gallery routes
                if (prevPathname.includes('/add-category') ||
                    prevPathname.includes('/edit-category') ||
                    prevPathname.includes('/view-category') ||
                    prevPathname.includes('/categories') ||
                    prevPathname.includes('/gallery')) {
                    prevShouldBeOpen = false;
                } else {
                    const prevIndex = prevPathname.split('/').findIndex((id) => id === props.collapse.id);
                    if (props.collapse.children) {
                        prevShouldBeOpen = props.collapse.children.some((child) => {
                            if (child.type === 'item' && child.url) {
                                const shouldMatchChildRoutes = [
                                    'all-events', 'upcoming-events', 'registered-events'
                                ].includes(child.id);
                                
                                if (shouldMatchChildRoutes) {
                                    return prevPathname.startsWith(child.url);
                                }
                                return prevPathname === child.url;
                            }
                            return false;
                        });
                    }
                    prevShouldBeOpen = prevShouldBeOpen || (prevIndex > -1 && prevIndex > 0);
                }
            } else {
                // For other collapses, normal check
                const prevIndex = prevPathname.split('/').findIndex((id) => id === props.collapse.id);
                if (props.collapse.children) {
                    prevShouldBeOpen = props.collapse.children.some((child) => {
                        if (child.type === 'item' && child.url) {
                            const shouldMatchChildRoutes = [
                                'users', 'speakers', 'exhibitors',
                                'surveys', 'polling', 'order ', 'engagements'
                            ].includes(child.id);
