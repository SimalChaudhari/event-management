...
export class AuthService {
  ...
  // Register a new user
  // isAdminRegistration: true if called from /register-admin endpoint, false if from /register endpoint
  async register(userDto: UserDto, isAdminRegistration: boolean = false): Promise<{ message: string }> {
    ...
    try {
      ...
      // Admin-created users: password is optional (will be auto-generated if not provided)
      // Self-registration: password is required
      let plainPassword: string;
      const wasPasswordAutoGenerated = !userDto.password || userDto.password.trim() === '';
      const isAdminCreatedUser = isAdminRegistration || wasPasswordAutoGenerated;
      ...
      if (wasPasswordAutoGenerated) {
        ...
        plainPassword = userDto.password!; // Safe to use ! here since we checked it's not empty above
      }
      ...
      // Create address if address data is provided (within transaction)
      ...
      // Create address using transaction manager to ensure it's part of the transaction
      ...
      if (hasAnyAddressData) {
        const addressModule = await import('../user/address.entity');
        const AddressEntity = addressModule.AddressEntity;
        const AddressTypeEnum = addressModule.AddressType;
        
        // If this is set as default, unset other default addresses for this user
        if (addressData.isDefaultAddress !== false) {
          await queryRunner.manager.update(
            AddressEntity,
            { userId: savedUser.id, isDefault: true },
            { isDefault: false }
          );
        }

        // Determine address type - use provided type if valid, otherwise default to HOME
        let addressTypeValue = AddressTypeEnum.HOME;
        if (addressData.addressType) {
          const validTypes = Object.values(AddressTypeEnum);
          if (validTypes.includes(addressData.addressType as any)) {
            addressTypeValue = addressData.addressType as any;
          }
        }

        const newAddress = queryRunner.manager.create(AddressEntity, {
          ...
          isDefault: addressData.isDefaultAddress !== false,
        ...
        await queryRunner.manager.save(AddressEntity, newAddress);
      }
      ...
      // Send email BEFORE committing transaction - if email fails, rollback
      if (isAdminCreatedUser && !userDto.password) {
        ...
        try {
          ...
          // Email sending failed - throw short, user-friendly error
          const errorCode = (emailError as any)?.code || 'EMAIL_ERROR';
          
          let userMessage = 'Account creation failed: Unable to send email. Please try again.';
          
          if (errorCode === 'ETIMEDOUT' || errorCode === 'ECONNREFUSED' || errorCode === 'ESOCKET') {
            userMessage = 'Account creation failed: Email server connection error.';
          } else if (errorCode === 'EAUTH') {
            userMessage = 'Account creation failed: Email authentication error.';
          } else if (errorCode === 'ETIMEDOUT') {
            userMessage = 'Account creation failed: Email sending timed out.';
          }
        ...
        // Admin created user with provided password - no email needed, commit transaction
        await queryRunner.commitTransaction();
        return {
        ...
        try {
          ...
          let userMessage = 'Registration failed: Unable to send verification email. Please try again.';
          
          if (errorCode === 'ETIMEDOUT' || errorCode === 'ECONNREFUSED' || errorCode === 'ESOCKET') {
            userMessage = 'Registration failed: Email server connection error.';
          } else if (errorCode === 'EAUTH') {
            userMessage = 'Registration failed: Email authentication error.';
          } else if (errorMessage.includes('timeout') || errorMessage.includes('TIMEDOUT')) {
            userMessage = 'Registration failed: Email sending timed out.';
          }
      ...
      // Rollback transaction on any error (including email errors)
